...
2. **Link every tool that renders the widget by setting its `_meta.ui.resourceUri`** to that same
   ...
   ```
   `mark3labs/mcp-go` exposes **no** capabilities-extensions hook, so there is nothing to advertise
   there — discovery relies solely on the tool `_meta.ui.resourceUri`, and the host attaches the App
   Bridge from the template's `text/html;profile=mcp-app` mimeType. Register these stable widget
   resources once at construction (e.g. a `registerWidgets` step alongside `registerTools`).
